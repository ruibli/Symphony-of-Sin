...
func _on_hit_cooldown_timeout():
	can_hit = true
	$wrathhurt.set_collision_layer_value(3,true)

func hit(ow):
	if can_hit:
		can_hit = false
		$wrathhurt.set_collision_layer_value(3,false)
		$HitCooldown.start()
		health -= ow
		
		var tween = get_tree().create_tween()
		tween.tween_property($WrathCollision/WrathAnimation, "modulate", Color(1, 0, 0, 1), 0.05)
		if health <= 0:
			await get_tree().create_timer(0.05).timeout
			Glova.g_enemies(-1)
			queue_free()
		else:
			tween.tween_property($WrathCollision/WrathAnimation, "modulate", Color(1, 1, 1, 1), 0.05)

func _on_wrathhit_hurt_area_entered(area):
		area.hit(damage)
