	...
	if active:
		...
		if type == "move":
			if velocity.y < 0 and abs(velocity.y) > abs(velocity.x): #up
				$PrideCollision/PrideAnimation.play("move_up")
			elif velocity.y > 0 and abs(velocity.y) > abs(velocity.x): #down
				$PrideCollision/PrideAnimation.play("move_down")
			elif velocity.x < 0 and abs(velocity.x) > abs(velocity.y): #left
				$PrideCollision/PrideAnimation.play("move_left")
			elif velocity.x > 0 and abs(velocity.x) > abs(velocity.y): #right
				$PrideCollision/PrideAnimation.play("move_right")
		elif type == "attack":
...
func _on_hit_cooldown_timeout():
	can_hit = true
	$pridehurt.set_collision_layer_value(3,true)

func hit(ow):
	if can_hit:
		can_hit = false
		$pridehurt.set_collision_layer_value(3,false)
		$HitCooldown.start()
		health -= ow
...
func _on_pridehurt_area_entered(area):
	area.hit(damage)
